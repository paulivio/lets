          ...
          >
            <div className="px-[18px] py-[14px] flex items-center justify-between" style={{ borderBottom: '1px solid var(--border)' }}>
              <div>
                ...
                <div className="text-[10px] mt-0.5" style={{ color: 'var(--muted)' }}>{sb.length} bookings · {activeRooms.length} rooms active</div>
              </div>
              <div className="px-2.5 py-0.5 rounded-full text-[10px]" style={{ background: 'var(--surface2)', border: '1px solid var(--border)', color: 'var(--accent2)' }}>
                {todayB.length} today
              </div>
            </div>
            ...
            <div className="pb-2">
              {activeRooms.length === 0 ? (
                <div className="px-[18px] py-4 text-[11px]" style={{ color: 'var(--muted)' }}>No bookings in this period</div>
              ) : (
                activeRooms.map((room) => {
                  ...
                  return (
                    <div key={room} className="px-[18px] py-1.5 flex items-center gap-2.5">
                      ...
                      <div className="w-[70px] text-[11px] whitespace-nowrap" style={{ color: 'var(--muted)' }}>{room}</div>
                      <div className="flex-1 h-[22px] rounded relative overflow-hidden" style={{ background: 'var(--surface2)' }}>
                        {rb.map((b) => {
                          ...
                          return (
                            <div
                              key={b.id}
                              className="absolute h-full rounded flex items-center justify-center text-[9px] font-medium text-black overflow-hidden cursor-default hover:opacity-80"
