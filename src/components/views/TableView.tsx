    ...
    <div>
      <div className="flex gap-2.5 mb-3.5 items-center flex-wrap">
        <input
          ...
          style={{ background: 'var(--surface)', border: '1px solid var(--border)', color: 'var(--text)', fontFamily: "'DM Mono', monospace" }}
        />
        <select
      ...
      <div className="overflow-x-auto">
        <table className="w-full" style={{ borderCollapse: 'collapse', border: '1px solid var(--border)', borderRadius: 8, overflow: 'hidden' }}>
          <thead>
            <tr style={{ background: 'var(--surface2)' }}>
              {COLUMNS.map(({ key, label }) => (
                <th
                  key={key}
                  className="px-3.5 py-2.5 text-left text-[10px] tracking-widest uppercase whitespace-nowrap select-none cursor-pointer"
                  style={{ color: state.sortKey === key ? 'var(--accent)' : 'var(--muted)', borderBottom: '1px solid var(--border)', fontFamily: "'DM Mono', monospace" }}
          ...
          <tbody>
            {rows.length === 0 ? (
              <tr><td colSpan={8} className="text-center py-12" style={{ color: 'var(--muted)' }}>No bookings match your filters</td></tr>
            ) : (
              rows.map((b) => (
                <tr
                  key={b.id}
                  ...
                  onMouseEnter={(e) => { e.currentTarget.style.background = 'var(--surface2)'; onHover(b, e) }}
                  onMouseLeave={(e) => { e.currentTarget.style.background = ''; onHover(null) }}
                >
